...
ann_accuracy = accuracy_score(y_test, ann_binary_predictions)
ann_roc_auc = roc_auc_score(y_test, ann_predictions)
ann_precision = precision_score(y_test, ann_binary_predictions)
ann_recall = recall_score(y_test, ann_binary_predictions)
ann_f1 = f1_score(y_test, ann_binary_predictions)

# Ensemble Model Predictions
rf_predictions = rf.predict_proba(X_test_scaled)[:, 1]
et_predictions = et.predict_proba(X_test_scaled)[:, 1]
ensemble_predictions = (rf_predictions + ann_predictions + et_predictions) / 3  # Average predictions
ensemble_binary_predictions = (ensemble_predictions > 0.5).astype(int)
ensemble_accuracy = accuracy_score(y_test, ensemble_binary_predictions)
ensemble_roc_auc = roc_auc_score(y_test, ensemble_predictions)
...
def get_model_metrics():
    metrics = {
        ...
        'ensemble_precision': precision_score(y_test, ensemble_binary_predictions),
        'ensemble_recall': recall_score(y_test, ensemble_binary_predictions),
        'ensemble_f1': f1_score(y_test, ensemble_binary_predictions),
        'random_forest_accuracy': accuracy_score(y_test, rf.predict(X_test_scaled)),
        'random_forest_roc_auc': roc_auc_score(y_test, rf.predict_proba(X_test_scaled)[:, 1]),
        'extra_trees_accuracy': accuracy_score(y_test, et.predict(X_test_scaled)),
        'extra_trees_roc_auc': roc_auc_score(y_test, et.predict_proba(X_test_scaled)[:, 1]),
    }
    return metrics
